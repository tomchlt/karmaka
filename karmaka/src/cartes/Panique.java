	...
	public void activerCapacite(Joueur joueur) {
		...
		if (joueurAdverse.getPile().isEmpty() == false) {
			...
			if (joueur instanceof JoueurVirtuel) {
				console.afficher(
						"Le Joueur Virtuel adverse vous fait défausser la première carte de votre Pile");
			}
			...
			if (joueur instanceof JoueurVirtuel) {
				console.afficher(
						"Vous n'avez pas de carte dans votre Pile que le Joueur Virtuel adverse puisse vous faire défausser");
			}
		}
		...
		// Si le joueur a choisi de jouer une autre carte, il choisit quelle carte jouer
		// et comment la jouer
		if (choixJouer == 1) {
			int choixCarte = -1;
			int choix = 0;
